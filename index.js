...
const SQL_PORT = 3306;
const SQL_USER = "freedb_c237_adminnn";
const SQL_PASS = "R&Zq38X#BKJBqn3";
const SQL_DATABASE = "freedb_C237_hosting";
...
// If a user accesses an endpoint with this middleware method, they must have the X-CSJS-RunOwner header before the server will return a response.
...
// First, Check that request contains the X-CSJS-RunOwner header. IF don't have, then reject request.
// Next, check that the user is authenticated by searching for "user" cookie. If the value is not "admin" or cookie is missing, reject request.
...
// Status endpoint will send a 200 or 503 response depending on MySQL database state. This only works if the 'X-CSJS-RunOwner' header is present.
// Otherwise, render the status.html page for a regular user.
